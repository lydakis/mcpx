...
_mcpx_completion() {
  ...
  if [[ "$first" == "skill" ]] && ! _mcpx_has_skill_server; then
    ...
    if [[ ${COMP_CWORD} -eq 3 ]] && [[ "${COMP_WORDS[2]}" == "install" ]]; then
      ...
      COMPREPLY=( $(compgen -W "$servers --data-agent-dir --claude-dir --no-claude-link --codex-dir --codex-link --kiro-dir --kiro-link --help -h" -- "$cur") )
      return 0
    fi
...
complete -c mcpx -n 'set -l w (__mcpx_words); test (count $w) -eq 4; and test "$w[2]" = skill; and not __mcpx_has_skill_server; and test "$w[3]" = install' -a "(mcpx __complete servers 2>/dev/null) --help -h --data-agent-dir --claude-dir --no-claude-link --codex-dir --codex-link --kiro-dir --kiro-link"
complete -c mcpx -n 'set -l w (__mcpx_words); test (count $w) -ge 5; and test "$w[2]" = skill; and not __mcpx_has_skill_server; and test "$w[3]" = install' -a "--data-agent-dir --claude-dir --no-claude-link --codex-dir --codex-link --kiro-dir --kiro-link --help -h"
complete -c mcpx -n 'set -l w (__mcpx_words); test (count $w) -ge 4; and test "$w[2]" = skill; and not __mcpx_has_skill_server; and test "$w[3]" != install' -a "--data-agent-dir --claude-dir --no-claude-link --codex-dir --codex-link --kiro-dir --kiro-link --help -h"
