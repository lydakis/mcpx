...
func printRootHelp(out io.Writer) {
	...
	fmt.Fprintln(out, "  mcpx import [<source> [<server>... | --all | --refresh]] [FLAGS]")
	...
	fmt.Fprintln(out, "                   mcpx, mcpx import, mcpx <server>, and tool --help")
	fmt.Fprintln(out, "")
	fmt.Fprintln(out, "Server listing flags (for `mcpx`):")
	fmt.Fprintln(out, "  --verbose, -v    Include server origin kind metadata")
	fmt.Fprintln(out, "")
